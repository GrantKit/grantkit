...
class GrantKitSync:
    ...
    def share(
        ...
        Args:
            ...
            role: Permission level - "viewer" (read-only) or "editor" (can edit).

        Returns:
            Dict with success status and details or error message.
        """
        # First, look up the user by email in profiles or auth
        try:
        ...
        if not user_data:
            return {
                ...
                "They must have a GrantKit account first.",
            }

        user_id = user_data["id"]

        # Insert into grant_collaborators table using RPC to bypass RLS issues
        try:
            ...
            if "duplicate" in error_msg or "unique" in error_msg:
                return {
                    "success": False,
                    "error": f"Grant is already shared with {email}",
                }
            return {
                "success": False,
                "error": f"Failed to share grant: {e}",
            }
    ...
    def unshare(
        ...
        """
        try:
            self.client.table("grant_collaborators").delete().eq(
                "grant_id", grant_id
            ).eq("user_email", email).execute()

            logger.info(f"Removed {email} from grant '{grant_id}'")
            return {"success": True, "email": email, "grant_id": grant_id}

        except Exception as e:
            return {
                "success": False,
                "error": f"Failed to remove collaborator: {e}",
            }

    def list_collaborators(self, grant_id: str) -> list:
        """
        ...
        Returns:
            List of collaborator dicts with email, role, and created_at.
        """
        try:
            result = (
