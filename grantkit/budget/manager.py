...
class GSAPerDiemAPI:
    ...
    def get_rates(
        ...
        try:
            ...
            params = {"fy": str(fiscal_year), "api_key": self.api_key}

            url_with_params = f"{url}?{urllib.parse.urlencode(params)}"

            with urllib.request.urlopen(
                url_with_params, timeout=10
            ) as response:
            ...
            # Get M&IE rate
            mie_rate = (
                float(rate_data.get("meals", 0))
                if rate_data.get("meals")
                else None
            )
            ...
            if month and "months" in rate_data:
                ...
                if 1 <= month <= 12:
                    ...
                    lodging_rate = (
                        float(monthly_data.get("lodging", 0)) or None
                    )
            ...
            logger.warning(
                f"Failed to fetch GSA rates for {city}, {state}: {e}"
            )
            return None, None
...
class BudgetManager:
    ...
    def _load_travel_items(self, items: List[Dict]) -> None:
        ...
        for item_data in items:
            travel = TravelItem(
                ...
                destination_city=item_data.get("destination", {}).get(
                    "city", ""
                ),
                destination_state=item_data.get("destination", {}).get(
                    "state", ""
                ),
                fiscal_year=int(
                    item_data.get("destination", {}).get(
    ...
    def _calculate_travel_cost(self, travel: TravelItem) -> None:
        ...
        per_person_total = (
            travel.airfare_per_person + lodging_total + mie_total
        )
    ...
    def calculate_totals(self) -> BudgetSummary:
        ...
        if not self.categories["I"]:
            indirect_item = BudgetItem(
                ...
                category="I",
                metadata={"rate": self.indirect_rate, "mtdc_base": mtdc_base},
            )
        ...
        if total_cost > self.budget_cap:
            issues.append(
                f"Budget exceeds cap by ${total_cost - self.budget_cap:,.0f}"
            )
    ...
    def generate_budget_narrative(
        self, output_path: Path, template_name: str = "budget_narrative.md"
    ) -> None:
