...
class URLValidationConfig:
    ...
    allowed_domains: List[str] = field(
        default_factory=lambda: [".gov", ".edu", "nsf.gov", "doi.org"]
    )
...
class ValidationConfig:
    ...
    # URL validation
    main_document: URLValidationConfig = field(
        default_factory=URLValidationConfig
    )
    references: ReferencesValidationConfig = field(
        default_factory=ReferencesValidationConfig
    )

    # Email validation
    prohibit_emails_everywhere: bool = True
    prohibited_email_sections: List[str] = field(
        default_factory=lambda: [
            "project_description",
            "references",
            "biographical_sketch",
...
class ReferencesConfig:
    ...
    def from_yaml(cls, config_data: Dict[str, Any]) -> "ReferencesConfig":
        ...
        try:
            ...
            main_doc_urls = URLValidationConfig(
                prohibit_all_urls=urls.get("main_document", {}).get(
                    "prohibit_all_urls", False
                ),
                allowed_domains=urls.get("main_document", {}).get(
                    "allowed_domains", [".gov", ".edu", "nsf.gov", "doi.org"]
                ),
                unknown_domain_severity=urls.get("main_document", {}).get(
                    "unknown_domain_severity", "warning"
                ),
            )

            refs_urls = ReferencesValidationConfig(
                allow_academic_urls=urls.get("references", {}).get(
                    "allow_academic_urls", True
                ),
                prohibit_cloud_storage=urls.get("references", {}).get(
                    "prohibit_cloud_storage", True
                ),
                prohibited_domains=urls.get("references", {}).get(
                    "prohibited_domains",
                    [
                        "dropbox.com",
                        "drive.google.com",
            ...
            validation_config = ValidationConfig(
                ...
                prohibit_emails_everywhere=emails.get(
                    "prohibit_everywhere", True
                ),
                prohibited_email_sections=emails.get(
                    "prohibited_sections",
                    [
                        "project_description",
                        "references",
            ...
            pdf_config = PDFReferencesConfig(
                ...
                number_format=pdf_formatting.get(
            ...
            return cls(
                default_filename=bibliography.get(
                    "default_filename", "references.bib"
                ),
                search_paths=bibliography.get(
                    "search_paths",
                    [
                        ".",
                        "references/",
    ...
    def load_from_file(cls, config_path: Path) -> "ReferencesConfig":
        ...
        try:
            ...
            logger.warning(
                f"Failed to load references configuration from {config_path}: {e}"
            )
            return cls()  # Return default configuration
    ...
    def load_default(cls) -> "ReferencesConfig":
        ...
        package_config = (
            Path(__file__).parent.parent / "data" / "references_config.yaml"
        )
        if package_config.exists():
            return cls.load_from_file(package_config)
        else:
            logger.warning(
                "Default references configuration not found, using built-in defaults"
            )
            return cls()
