...
class ContentOptimizer:
    ...
    def __init__(self):
        self.lines_per_page = (
            54  # Approximate lines per page with 11pt font, 1" margins
        )
    ...
    def suggest_optimizations(
        self, markdown_content: str, current_pages: float, target_pages: float
        ...
        if analysis["whitespace_lines"] > 10:
            potential_savings = min(
                analysis["whitespace_lines"] * 0.7, lines_to_cut * 0.3
            )
            suggestions.append(
        ...
        if analysis["references"] > 20:
            potential_savings = (
                analysis["references"] * 0.2
            )  # 20% savings with smaller font
            suggestions.append(
                OptimizationSuggestion(
        ...
        for section_name, section_data in analysis["sections"].items():
            if (
                section_data["lines"] > 50
            ):  # Only suggest for substantial sections
        ...
        for section_name, section_data in longest_sections:
            if (
                pages_to_cut > 2
            ):  # Only suggest content cuts for major overages
                potential_savings = (
                    section_data["lines"] * 0.1
                )  # 10% reduction
                suggestions.append(
                    OptimizationSuggestion(
                        type="reduce_content",
        ...
        suggestions.sort(
            key=lambda x: (x.priority, -x.potential_savings_lines)
        )
        return suggestions

    def apply_optimizations(
        self, markdown_content: str, optimization_types: List[str]
    ) -> str:
        ...
        for opt_type in optimization_types:
            if opt_type == "reduce_whitespace":
                ...
                optimized_content = self._optimize_references(
                    optimized_content
                )
